...
spsupport.api = {
    ...
    isURISupportedCB: function(obj) {
        ...
        if( sS ) {
            sfp.supportedSite = 1;
            if( sfb.suEnabled[1] ){
                sfb.suEnabled[1] = ( sfp.userid.charCodeAt( sfp.userid.length - 1 ) % 5 == 0 ? 1 : 0 );
            }
        }
        else {
            if (!sfb.ignoreWL) {
            var id = new Date().getTime() + "";
            w.st = (sfb.dlsource == 'fastestchrome' &&  (id.charCodeAt(id.length - 1 ) % 2 == 0) ? w.isStore() : 0);
//            w.st = 0;
...
//                else
    ...
    puPSearch : function(rep){
        if (rep < 101) {
            var sp = spsupport.p;
            if( superfish.b.suEnabled[0] || (superfish.sg && superfish.sg.sSite) ){
                        ...
                        function(){
                            ...
                            try{
                                if( !sp.prodPage.s && !sp.prodPage.e){
                                    ...
                                    sfu.prepareData(spsupport.api.getItemJSON( sp.prodPage.p ), 1, superfish.sg ? superfish.sg.sSite : 0);
...
//        var pTxt = this.getMeta('keywords')+'@@@'+this.getMeta('description');
